...
const paymentBadge = (o) => {
  ...
  if (pm === "exchange")
    return { label: "Exchange", cls: "bg-blue-50 text-blue-700" };
  if (pm === "cod")
    return { label: "COD", cls: "bg-black/5 text-black/80" };
  if (ps === "paid")
    return { label: "Paid", cls: "bg-emerald-50 text-emerald-700" };
  if (ps === "pending")
    return { label: "Pending", cls: "bg-amber-50 text-amber-700" };
  if (ps === "refunded")
    return { label: "Refunded", cls: "bg-purple-50 text-purple-700" };
...
function RmaCreatePanel({ order }) {
  ...
  const submit = async () => {
    ...
    if (type === "exchange") {
      ...
      if (!SIZE_OPTIONS.includes(newSize)) {
        return toast.error("Invalid size (only XS/S/M/L/XL)");
      }
  ...
  return (
    <div className={`mt-3 rounded-2xl ${b2} ${card} p-3`}>
      ...
      {type === "return" ? (
        <div className="mt-3 rounded-2xl bg-black/[0.02] border border-black/5 p-3">
          <div className="flex flex-wrap items-center gap-3">
            <div>
              ...
              <div className="text-xs text-black/50">
                Choose how customer wants refund after approval.
              </div>
            </div>

            <select
              className="ml-auto min-w-[230px] rounded-xl px-3 py-2 text-sm bg-white border border-black/10 focus:outline-none focus:ring-2"
              style={{ outlineColor: ACCENT }}
          ...
          <div className="mt-2 text-xs text-black/50">
            ...
            <span className="font-semibold text-black/80">
              {labelize(refundPreference)}
            </span>
          </div>
        </div>
      ) : null}

      <div className="mt-3 grid grid-cols-12 gap-2 text-xs font-medium text-black/55 px-1">
          ...
          return (
            ...
            >
              <div className="grid grid-cols-12 gap-2 items-center py-2">
                ...
                <div className="col-span-2 text-sm text-black/75">
                  {lineId || "-"}
                </div>

                <div className="col-span-1 text-sm text-black/75">
                  {bought}
                </div>
                ...
                <div className="col-span-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setQtyByLineId((s) => ({
                        ...(s || {}),
                        [lineId]: Number.isFinite(v)
                          ? Math.min(Math.max(v, 0), bought)
                          : 0,
                      }));
                    }}
                  />
                ...
                <div className="pb-2 pl-12 pr-2">
                  <div className="grid grid-cols-12 gap-2 items-end">
                    ...
                    <div className="col-span-12 md:col-span-6 text-xs text-black/45">
                      Same product exchange. Backend will resolve correct variant
                      by size.
                    </div>
                  </div>
                </div>
  ...
  return (
    <div className={`${bg} min-h-screen p-4 md:p-6`}>
      <div className="flex flex-wrap items-center gap-2">
        <div>
          ...
          <div className="text-xs text-black/50 mt-1">
            Create return or exchange requests for delivered orders.
          </div>
        </div>
      ...
      <div className="mt-2 text-sm text-black/60">
        Showing{" "}
        <span className="font-semibold text-black/80">{list.length}</span>{" "}
        delivered orders.
        {hasMore ? (
          <span className="ml-2 text-xs text-black/45">(more available)</span>
        ) : null}
      </div>
                  ...
                  <tbody key={id}>
                    <tr className="border-t border-black/5">
                      ...
                      <td className="p-3 align-top">
                        ...
                        <div
                          className={`mt-1 inline-flex px-2 py-1 rounded-full text-xs ${statusPill(
                            o?.fulfillmentStatus
                          )}`}
                        >
                      ...
                      <td className="p-3 align-top">
                        <span
                          className={`inline-flex px-2 py-1 rounded-full text-xs ${pay.cls}`}
                        >
                          {pay.label}
                        </span>
                      </td>

                      <td className="p-3 align-top text-black/80">
                        {itemsCount}
                      </td>

                      <td className="p-3 align-top text-black/70">
                        {fmtDT(deliveredAt)}
                      </td>
                      ...
                      <tr className="border-t border-black/5 bg-black/[0.02]">
                        <td className="p-3" colSpan={7}>
                          <div className={`rounded-3xl ${b2} ${card} p-3`}>
                            ...
                            <div
                              className={`mt-2 rounded-3xl ${b2} overflow-hidden`}
                            >
