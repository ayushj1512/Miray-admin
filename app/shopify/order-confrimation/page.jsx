...
const Badge = ({ className = "", children }) => (
  <span className={`text-xs px-2 py-1 rounded-full border font-semibold ${className}`}>
    {children}
  </span>
);
    ...
    <div className="flex gap-3 p-3 rounded-xl border border-gray-200 bg-white">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <Badge className="bg-gray-50 border-gray-200 text-gray-700">Qty: {qty || 1}</Badge>
          <Badge className="bg-blue-50 border-blue-200 text-blue-700">₹ {money(price)}</Badge>
...
export default function ShopifyOrderConfirmationPage() {
  const {
    orders,
    ordersMeta,
    loading,
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      setLocalError(e?.message || "Failed to fetch Shopify orders");
    }
  ...
  const loadMore = useCallback(async () => {
    try {
      ...
      setLocalError(e?.message || "Failed to load more Shopify orders");
    } finally {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return (localOrders || [])
      .filter((o) => {
        ...
        if (safe(o.source).toLowerCase() !== "shopify") return false;
        ...
        if (q) {
          ...
          const shopifyName = safe(o.shopify?.orderName).toLowerCase();
          const shopifyId = safe(o.shopify?.orderId).toLowerCase();
          ...
          if (
            ...
            !shopifyName.includes(q) &&
            !shopifyId.includes(q) &&
            !email.includes(q) &&
            !phone.includes(q)
          ) {
            return false;
  ...
  }, [localOrders, search, paymentStatus, minAmt, maxAmt, city, state, fromDate, toDate]);
  ...
  const toggleExpand = async (orderIdStr) => {
    try {
      ...
      setLocalError(e?.message || "Failed to load Shopify order detail");
    } finally {
  ...
  const handleConfirm = async (orderIdStr) => {
    ...
    try {
      ...
      setSuccessMsg("✅ Shopify COD Order Confirmed Successfully!");
    } catch (e) {
      setLocalOrders(prev);
  ...
  const handleCancelConfirm = async (reason = "") => {
    ...
    try {
      ...
      setSuccessMsg("❌ Shopify Order Cancelled!");
    } catch (e) {
      setLocalOrders(prev);
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-2 sm:px-6 py-3 sm:py-6">
      <div className="mx-auto space-y-3 sm:space-y-5">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-[11px] sm:text-sm text-gray-600 mt-0.5">
              Shopify only • COD + <b>processing/packed</b> • confirmed hidden
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-2 sm:gap-3">
            <div className="relative col-span-2 lg:col-span-2">
              ...
              <input
                ...
                placeholder="Search order/shopify/email/phone"
                className="w-full pl-9 pr-3 py-2 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-green-200 text-sm"
              />
            </div>
        ...
        {loading && (
          <div className="text-center text-gray-500 py-10">Loading Shopify orders…</div>
        )}

        {!loading && filteredOrders.length === 0 && (
          <div className="text-center text-gray-500 py-12 bg-white border border-gray-200 rounded-2xl shadow-sm">
            ✅ No Shopify COD confirmation orders found.
          </div>
        )}
        ...
        <div className="space-y-2.5 sm:space-y-3">
          <AnimatePresence>
            {filteredOrders.map((order, idx) => {
              ...
              const detailOrder = customerSupportOrderDetails?.[orderIdStr] || order;
              ...
              const shopify = detailOrder?.shopify || {};
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-1.5">
                        <div className="font-bold text-gray-900 text-sm sm:text-base">
                          {safe(shopify.orderName) || safe(order.orderNumber)}
                        </div>
                      ...
                      <div className="text-[11px] sm:text-xs text-gray-500 mt-1">
                        Local: <b>{safe(order.orderNumber) || "-"}</b> • Shopify ID:{" "}
                        <b>{safe(shopify.orderId) || "-"}</b>
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <button
                        ...
                        disabled={loading || isCancelLoading || isConfirmLoading || cancelLoading}
                        ...
                        {isDetailLoading ? (
                          ...
                          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-2.5 sm:p-4 space-y-3">
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-2.5">
                              <div className="rounded-xl bg-white border border-gray-200 p-2.5 sm:p-3">
                                ...
                                <div className="mt-2 text-sm text-gray-700">
                                  ...
                                  <div className="text-xs text-gray-500 mt-1 break-words">
                                    {safe(detailOrder?.customerId?.email) || safe(ship.email) || "N/A"}
                                    {" • "}
                                    {safe(ship.phone) || "N/A"}
                                  </div>
                              ...
                              <div className="rounded-xl bg-white border border-gray-200 p-2.5 sm:p-3">
                                ...
                                <div className="mt-3 text-xs text-gray-600 space-y-1">
                                  <div>Subtotal: <b>₹ {money(detailOrder?.subtotal)}</b></div>
                                  <div>Discount: <b>₹ {money(detailOrder?.discount)}</b></div>
                                  <div>Shipping: <b>₹ {money(detailOrder?.shippingFee)}</b></div>
                                  <div>Tax: <b>₹ {money(detailOrder?.tax)}</b></div>
                              ...
                              <div className="rounded-xl bg-white border border-gray-200 p-2.5 sm:p-3">
                                ...
                                <div className="mt-2 text-xs text-gray-600 space-y-1">
                                  <div>Provider: <b>{safe(shipment.provider) || "-"}</b></div>
                                  <div>Status: <b>{safe(shipment.status) || "-"}</b></div>
                                  <div>
                                    AWB:{" "}
                            ...
                            <div className="rounded-xl bg-white border border-gray-200 p-2.5 sm:p-3">
                              ...
                              <div className="mt-2 text-[11px] sm:text-xs text-gray-600 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                                <div>Order Name: <b>{safe(shopify.orderName) || "-"}</b></div>
                                <div>Order ID: <b>{safe(shopify.orderId) || "-"}</b></div>
                                <div>Financial: <b>{safe(shopify.financialStatus) || "-"}</b></div>
                                <div>Fulfillment: <b>{safe(shopify.fulfillmentStatus) || "-"}</b></div>
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-2.5">
                              <EditableAddressCard
                                ...
                                address={ship}
                                onRefresh={() =>
                                  fetchCustomerSupportOrderDetail(orderIdStr, { force: true })
                                }
                              />
                            ...
                            <div className="rounded-xl bg-white border border-gray-200 p-2.5 sm:p-3">
                              ...
                              <div className="mt-2 text-[11px] sm:text-xs text-gray-500">
                                Internal note. Customer will not see it.
                              </div>
                            </div>
                            ...
                            <div className="rounded-xl bg-white border border-gray-200 p-2.5 sm:p-3">
                              ...
                              <div className="mt-2 text-[11px] sm:text-xs text-gray-600 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                                <div>Local ID: <b className="text-gray-800">{safe(orderIdStr) || "-"}</b></div>
                                <div>Created: <b className="text-gray-800">{fmtDate(detailOrder?.createdAt)}</b></div>
                                <div>Updated: <b className="text-gray-800">{fmtDate(detailOrder?.updatedAt)}</b></div>
                                <div>Confirmed: <b className="text-gray-800">{fmtDate(detailOrder?.confirmedAt)}</b></div>
                                <div>Source: <b className="text-gray-800">{safe(detailOrder?.source) || "-"}</b></div>
                                <div>Currency: <b className="text-gray-800">{safe(detailOrder?.currency) || "INR"}</b></div>
                              </div>
                            </div>
                          </div>
                        )}
