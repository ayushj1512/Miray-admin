...
const normalizeOrderItem = (item = {}, index = 0) => {
  ...
  /* =========================================================
     ✅ ACTUAL INVENTORY RESERVATION FROM BACKEND
  ========================================================= */

  const reservation =
    item?.inventoryReservation &&
      typeof item.inventoryReservation === "object"
      ? item.inventoryReservation
      : {};

  const reservationStatus = toText(
  ...
  return {
    ...
    // ✅ actual reservation returned by backend
    inventoryReservation: {
      ...
      shippedQty: toNumber(
        reservation?.shippedQty,
        shippedQty
      ),

      toProduceQty: toNumber(
        reservation?.toProduceQty,
        toProduceQty
      ),
          ...
          reservationStatus === "reserved"
            ...
            : reservationStatus === "pending"
              ...
              : reservationStatus === "partial"
                ? `Partial ${reservedQty}/${requiredQty}`
...
export const normalizeOrder = (order = {}) => {
  ...
  /* =========================================================
     ✅ RESERVATION SUMMARY
     Backend summary preferred, otherwise calculate fallback
  ========================================================= */
  ...
  const totalRequiredQty = items.reduce(
    (sum, item) =>
      sum +
      Number(
        item?.inventoryReservation?.requiredQty ??
        item?.quantity ??
        0
      ),
    0
  ...
  const totalReservedQty = items.reduce(
    (sum, item) =>
      sum +
      Number(
        item?.inventoryReservation?.reservedQty ??
        0
      ),
    0
  );

  const fullyReservedItems = items.filter(
    (item) =>
      item?.inventoryReservation?.isFullyReserved === true
  ).length;

  let fallbackReservationStatus = "not_reserved";

  if (
    items.length > 0 &&
    fullyReservedItems === items.length
  ) {
    fallbackReservationStatus = "reserved";
  } else if (totalReservedQty > 0) {
    fallbackReservationStatus = "partial";
  }

  const inventoryReservationSummary = {
    status:
      order?.inventoryReservationSummary?.status ||
      fallbackReservationStatus,

    totalItems:
      Number(
        order?.inventoryReservationSummary?.totalItems
      ) || items.length,

    fullyReservedItems:
      Number(
        order?.inventoryReservationSummary?.fullyReservedItems
      ) || fullyReservedItems,

    totalRequiredQty:
      Number(
        order?.inventoryReservationSummary?.totalRequiredQty
      ) || totalRequiredQty,

    totalReservedQty:
      Number(
        order?.inventoryReservationSummary?.totalReservedQty
      ) || totalReservedQty,

    isFullyReserved:
      order?.inventoryReservationSummary?.isFullyReserved ??
      (
        items.length > 0 &&
        fullyReservedItems === items.length
      ),
  ...
  return {
    ...
    invoiceUrl:
      order?.shipment?.eshipz?.invoiceUrl || "",

    manifestUrl:
      order?.shipment?.eshipz?.manifestUrl || "",
  };
};
  ...
  searchProductOrders: async (q) => {
    ...
    const results = Array.isArray(data?.orders)
      ? data.orders.map((order) => ({
        ...
        source:
          String(order?.source || "").trim().toLowerCase() === "shopify"
            ? "shopify"
            : "website",
        ...
        inventoryStatuses: Array.isArray(order?.inventoryStatuses)
          ? [
            ...new Set(
    ...
    set({
      productOrderSearchResults: results,
    });
